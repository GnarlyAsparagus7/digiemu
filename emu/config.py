...
def plusdrive_image():
    """-> the +Drive (eMMC) image file, or None for an in-memory card.

    DT2_PLUSDRIVE names it. The file is created on first use and grows only
    as far as the firmware writes (it is memory-mapped, sparse on disk), so
    a 3.7 GB card costs nothing until it holds something."""
    return os.environ.get('DT2_PLUSDRIVE') or None
