...
def _run_mac_modes_case(factory):
    """Signed fractional and signed integer MAC and store (MCF5441x RM 5.3.5).

    Digitakt mk1's audio engine computes at MACSR 0xA0 (signed fractional)
    and mixes at 0x80 (signed integer, saturating). Stock Unicorn multiplies
    fractions unsigned and drops the product's << 1, so 0.970 * 0.5 came back
    as 0.2425; its fractional store shifted logically, so with OMC every
    negative result was stored as 0; and it has MACSR[S/U] backwards, so
    0x80 ran unsigned and every negative mix saturated to 0.
    """
    from unicorn import UcError
    from unicorn.m68k_const import (
        UC_M68K_REG_D1,
    ...
    for macsr, d5, d4, want in (
        (0xA0, 0x7C290000, 0x40000000, 0x3E148000),   # 0.970 * 0.5 = 0.485
        (0xA0, 0xC0000000, 0x40000000, 0xE0000000),   # -0.5 * 0.5 = -0.25
        (0xA0, 0xC0000000, 0xC0000000, 0x20000000),   # -0.5 * -0.5 = 0.25
        (0x80, 0xFFFFFC18, 0x00000001, 0xFFFFFC18),   # -1000 * 1, signed int
    ):
        uc = factory()
        uc.mem_map(0, 0x10000)
        uc.mem_write(0x1000, bytes.fromhex("a907 a1c1 a805 0800 a1c1"))
        uc.reg_write(UC_M68K_REG_SR, 0x2700)
        for regid, value in ((UC_M68K_REG_D7, macsr), (UC_M68K_REG_D5, d5),
                             (UC_M68K_REG_D4, d4)):
            uc.reg_write(regid, value)
        try:
            uc.emu_start(0x1000, 0x100A, count=4)
            ...
            return {"error": str(exc), "pass": False}
        got.append((uc.reg_read(UC_M68K_REG_D1) & 0xFFFFFFFF, want))
    return {"results": [[g, w] for g, w in got],
            "pass": all(g == w for g, w in got)}
...
def evaluate(factory=None):
    ...
    cases = {
        ...
        "emac_modes": _run_mac_modes_case(factory),
    }
