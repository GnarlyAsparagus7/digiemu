...
class Ssi0DmaTest(unittest.TestCase):
    ...
    def test_rx_source_delivers_what_it_supplies(self):
        """With a peer supplying the FIFO, the bytes reach the destination.

        The test above pins the opposite case. Together they say the receive
        channel invents nothing and drops nothing: measured on the real
        firmware, feeding a sine in put 128 of 128 non-zero words into the
        buffer at 0x80001000, where without this it stayed entirely zero
        after 3 MB had been fed.
        """
        machine, source = self.make_source()
        machine.uc.mem_write(0x5000, bytes(32))
        put_tcd(machine, RX_CHAN, source=RX_REGISTER, dest=0x5000, citer=2)
        source.enabled.add(RX_CHAN)
        frame = bytes((0xDE, 0xAD, 0xBE, 0xEF))
        source.rx_source = lambda n: frame * (n // len(frame))

        source._run_minor(RX_CHAN, capture_tx=False)

        # One minor loop is 0x20 bytes of 4-byte beats, all delivered.
        self.assertEqual(machine.uc.mem_read(0x5000, 32), frame * 8)
        self.assertEqual(source.rx_bytes, 0x20)
        self.assertEqual(source._u32(RX_CHAN, DADDR), 0x5020)

    def test_rx_source_none_still_fabricates_nothing(self):
        machine, source = self.make_source()
        before = bytes(range(32))
        machine.uc.mem_write(0x5000, before)
        put_tcd(machine, RX_CHAN, source=RX_REGISTER, dest=0x5000, citer=2)
        source.enabled.add(RX_CHAN)
        self.assertIsNone(source.rx_source)

        source._run_minor(RX_CHAN, capture_tx=False)

        self.assertEqual(machine.uc.mem_read(0x5000, 32), before)
        self.assertEqual(source.rx_bytes, 0)
